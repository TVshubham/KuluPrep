...
function operations(actions, values) {
    var calculator = new Calculator(values[0]);
    ...
    for (let i = 1; i < actions.length; i++) {
        ...
        if (action === "add")
            ...
            calculator.subtract(values[i]);
        else if (action === "multiply")
            calculator.multiply(values[i]);
        else if (action === "divide")
            calculator.division(values[i]);
        else if (action === "power")
            calculator.power(values[i]);
...
let actions = ["Calculator", "divide", "getResult"];
let values = [20, 0];
operations(actions, values);
